...
type result struct {
	...
	DevicePluginRefresh map[string]any `json:"devicePluginRefresh,omitempty"`
}
...
var migProcessLineRe = regexp.MustCompile(`^\|\s+([0-9]+)\s+([0-9]+)\s+([0-9]+)\s+([0-9]+)\s+([CG])\s+(.+?)\s+\|$`)
...
func main() {
	...
	flag.BoolVar(&httpAPI, "http-api", false, "run the node-local MIG actuator HTTP API")
	flag.StringVar(&httpAddr, "addr", ":10684", "HTTP API listen address")
	...
	flag.DurationVar(&scanInterval, "slot-scan-interval", time.Second, "MIG slot device plugin rescan interval")
	flag.Parse()

	if httpAPI {
		if err := runHTTPAPI(httpAddr, lockPath); err != nil {
...
func runHTTPAPI(addr, lockPath string) error {
	mux := http.NewServeMux()
	mux.HandleFunc("/healthz", func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(w, http.StatusOK, map[string]any{"ok": true, "component": "mig-node-agent"})
	})
	mux.HandleFunc("/list", func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := run("nvidia-smi", "-L")
		res := result{Command: "list", GPUIndex: gpuIndex, Success: err == nil, NvidiaSMIL: out}
		if err != nil {
			res.Message = err.Error()
		...
		if giErr == nil {
			res.MIGSlots = migSlotsFromObservation(out, instances, gpuIndex)
		} else {
			res.Message = strings.TrimSpace("nvidia-smi mig -lgi failed: " + giErr.Error() + "\n" + rawGI)
		}
	...
	mux.HandleFunc("/apply-slots", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			writeJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "method not allowed"})
			return
		}
		gpuIndex := queryDefault(r, "gpuIndex", "0")
		createSpec := r.URL.Query().Get("create")
		if createSpec == "" {
			var body struct {
	...
	mux.HandleFunc("/patch-slots", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			writeJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "method not allowed"})
			return
		}
		gpuIndex := queryDefault(r, "gpuIndex", "0")
		deleteSpec := r.URL.Query().Get("delete")
		createSpec := r.URL.Query().Get("create")
		preserveSpec := r.URL.Query().Get("preserve")
		if deleteSpec == "" && createSpec == "" && preserveSpec == "" {
			var body struct {
	...
	mux.HandleFunc("/refresh-cdi", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			writeJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "method not allowed"})
			return
		}
		gpuIndex := queryDefault(r, "gpuIndex", "0")
		res := withLock(lockPath, "refresh-cdi", gpuIndex, func() result {
			return refreshCDI(gpuIndex)
		})
		res = attachDevicePluginRefresh(res)
		writeResult(w, res)
	})
	mux.HandleFunc("/processes", func(w http.ResponseWriter, r *http.Request) {
		...
		if migUUID != "" {
			processes, raw, location, err := activeComputeProcessesForMIGUUID(migUUID)
			if err != nil {
				writeJSON(w, http.StatusInternalServerError, map[string]any{"success": false, "migUuid": migUUID, "raw": raw, "message": err.Error()})
				return
			}
			writeJSON(w, http.StatusOK, map[string]any{"success": true, "migUuid": migUUID, "location": location, "processes": processes, "processCount": len(processes), "raw": raw})
			return
		}
		activeByUUID, raw, err := activeComputeProcessesByUUID()
		if err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]any{"success": false, "raw": raw, "message": err.Error()})
			return
		}
		{
			writeJSON(w, http.StatusOK, map[string]any{"success": true, "processesByUUID": activeByUUID, "raw": raw})
			return
		}
	})
...
func refreshCDI(gpuIndex string) result {
	...
	for _, pair := range [][2]string{
		...
		{"hostPath: /host/etc", "hostPath: /etc"},
	} {
...
func activeComputeProcessesForMIGUUID(migUUID string) ([]string, string, map[string]string, error) {
	...
	for _, line := range strings.Split(out, "\n") {
		...
		if match[1] != gpuIndex || match[2] != giID {
			continue
		}
		if ciID != "" && match[3] != ciID {
			continue
		}
...
const (
	...
	refreshSocketName   = "or-sim-refresh.sock"
)
...
func runSlotDevicePlugin(nodeName, pluginDir string, scanInterval time.Duration) error {
	...
	if err := cleanupOrSimPluginSockets(pluginDir); err != nil {
		return err
	}
	var mu sync.Mutex
	scan := func(reason string) ([]slotDevice, error) {
		...
		for _, slot := range slots {
			...
			if !ok {
				...
				fmt.Fprintf(os.Stderr, "or-sim slot device plugin registered %s uuid=%s reason=%s\n", slot.ResourceName, slot.MIGUUID, reason)
			}
			server.update(slot)
		}
	...
	if _, err := scan("initial"); err != nil {
		fmt.Fprintf(os.Stderr, "or-sim slot device initial discovery failed: %v\n", err)
	}
	go serveDevicePluginRefresh(pluginDir, scan)
...
func serveDevicePluginRefresh(pluginDir string, scan func(reason string) ([]slotDevice, error)) {
	socketPath := filepath.Join(pluginDir, refreshSocketName)
	_ = os.Remove(socketPath)
	listener, err := net.Listen("unix", socketPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "or-sim device plugin refresh socket failed: %v\n", err)
		return
	}
	...
	mux.HandleFunc("/refresh", func(w http.ResponseWriter, r *http.Request) {
		...
		start := time.Now()
		slots, err := scan("active-refresh")
		body := map[string]any{
			"success":             err == nil,
			"seconds":             time.Since(start).Seconds(),
			"registeredResources": slotDeviceResources(slots),
			"slotCount":           len(slots),
		}
		if err != nil {
	...
	if err := http.Serve(listener, mux); err != nil {
		fmt.Fprintf(os.Stderr, "or-sim device plugin refresh server stopped: %v\n", err)
	}
}

func cleanupOrSimPluginSockets(pluginDir string) error {
	...
	for _, entry := range entries {
		name := entry.Name()
		if (strings.HasPrefix(name, "or-sim-io-") && strings.HasSuffix(name, ".sock")) || name == refreshSocketName {
...
func attachDevicePluginRefresh(res result) result {
	res = attachCurrentMIGSlots(res)
	res.ExpectedResources = expectedResourcesFromMIGSlots(res.MIGSlots)
	refresh := notifyDevicePluginRefresh()
	if len(refresh) > 0 {
		res.DevicePluginRefresh = refresh
	}
	return res
}

func attachCurrentMIGSlots(res result) result {
	if len(res.MIGSlots) > 0 || res.GPUIndex == "" {
		return res
	}
	smi, smiErr := run("nvidia-smi", "-L")
	if smiErr == nil && res.NvidiaSMIL == "" {
		res.NvidiaSMIL = smi
	}
	instances, _, giErr := listGPUInstances(res.GPUIndex)
	if smiErr == nil && giErr == nil {
		res.MIGSlots = migSlotsFromObservation(smi, instances, res.GPUIndex)
	}
	return res
}

func notifyDevicePluginRefresh() map[string]any {
	socketPath := filepath.Join(deviceplugin.DevicePluginPath, refreshSocketName)
	if _, err := os.Stat(socketPath); err != nil {
		hostSocketPath := filepath.Join("/host", deviceplugin.DevicePluginPath, refreshSocketName)
		if _, hostErr := os.Stat(hostSocketPath); hostErr != nil {
			return map[string]any{"success": false, "socket": socketPath, "hostSocket": hostSocketPath, "error": err.Error(), "hostError": hostErr.Error()}
		}
	...
	resp, err := client.Post("http://or-sim-device-plugin/refresh", "application/json", nil)
	if err != nil {
		return map[string]any{"success": false, "socket": socketPath, "error": err.Error()}
	}
	defer resp.Body.Close()
	var out map[string]any
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return map[string]any{"success": false, "socket": socketPath, "statusCode": resp.StatusCode, "error": err.Error()}
	}
...
func expectedResourcesFromMIGSlots(slots []migSlot) []string {
	...
	for _, slot := range slots {
		...
		resource := migUUIDResourceName(slot.MIGDeviceUUID)
		if seen[resource] {
			continue
		}
		seen[resource] = true
...
func slotDeviceResources(slots []slotDevice) []string {
	out := make([]string, 0, len(slots))
	for _, slot := range slots {
		out = append(out, slot.ResourceName)
	}
	sort.Strings(out)
	return out
}

func possibleSlotDevices(nodeName string) ([]slotDevice, error) {
	smi, err := run("nvidia-smi", "-L")
	if err != nil {
		return nil, err
	}
...
func (s *slotResourceServer) markUnavailable() {
	device := s.service.current()
	device.MIGUUID = ""
	s.service.set(device)
}
...
func (s *slotDevicePluginServer) ListAndWatch(_ *deviceplugin.Empty, stream deviceplugin.DevicePlugin_ListAndWatchServer) error {
	...
	for {
		...
		if device.MIGUUID != "" {
			devices = append(devices, &deviceplugin.Device{
				ID:     device.MIGUUID,
		...
		if err := stream.Send(resp); err != nil {
			fmt.Fprintf(os.Stderr, "or-sim slot device plugin ListAndWatch ended for %s: %v\n", device.ResourceName, err)
			return err
		}
		<-ticker.C
	}
...
func (s *slotDevicePluginServer) Allocate(_ context.Context, req *deviceplugin.AllocateRequest) (*deviceplugin.AllocateResponse, error) {
	device := s.current()
	if device.MIGUUID == "" {
		return nil, fmt.Errorf("or-sim slot %s is not currently backed by a MIG UUID", device.ResourceName)
	}
	responses := make([]*deviceplugin.ContainerAllocateResponse, 0, len(req.ContainerRequests))
	for _, containerReq := range req.ContainerRequests {
		if len(containerReq.DevicesIDs) != 1 {
			return nil, fmt.Errorf("or-sim slot %s expected exactly one device allocation, got %v", device.ResourceName, containerReq.DevicesIDs)
		}
		responses = append(responses, &deviceplugin.ContainerAllocateResponse{
			Envs: map[string]string{
				"NVIDIA_VISIBLE_DEVICES":     device.MIGUUID,
...
func parseGPUNames(smi string) map[string]string {
	re := regexp.MustCompile(`^GPU ([0-9]+): (.+) \(UUID: [^)]+\)$`)
	out := map[string]string{}
	for _, line := range strings.Split(smi, "\n") {
		match := re.FindStringSubmatch(strings.TrimSpace(line))
...
func isMIGCapableA100Name(name string) bool {
	name = strings.ToUpper(name)
	return strings.Contains(name, "A100")
}
...
func migUUIDResourceName(migUUID string) string {
	...
	return fmt.Sprintf("%s/mig-%s", orSimResourceDomain, resourceToken(uuid))
}
