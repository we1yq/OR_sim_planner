...
def build_initial_free_pool(state: ClusterState) -> list[str]:
    ...
    observed_free_pool = [
        str(physical_id)
        for physical_id in state.metadata.get("free_physical_gpu_pool", [])
        if physical_id is not None and str(physical_id) not in active
    ]
    if str(state.metadata.get("source", "")).startswith("go-cluster-state-manager"):
        return list(reversed(observed_free_pool))
...
def alloc_from_free_pool(free_pool: list[str]) -> str:
    if not free_pool:
        raise RuntimeError("Out of free physical GPUs")
...
def target_geometry_only_gpu(target_gpu: GPUState) -> GPUState:
    ...
    for action in fine_actions:
        ...
        if action_type == "bind_target_gpu":
            ...
            if target_gpu is None:
                continue
            replace_or_append_gpu(executed_state, target_geometry_only_gpu(target_gpu))
            executed_state.metadata["physical_id_map"][gpu_id] = physical_id
            continue
