...
from .dag_format import build_phased_action_plan, compact_phased_action_plan
...
def run(
    *,
    ...
    stage_name: str = "stage_action_builder",
    max_iters: int = 1,
    ...
    **_: Any,
    ...
    This function is kept only for local helper validation. The production
    planner catalog exposes effect_aware_dag, which reuses this module's action
    lowering helpers.
    """
...
def _build_final_actions(
    *,
    ...
    for gpu_id in reconfig_ids:
        ...
        if partial_plan is not None and _partial_reconfiguration_capacity_safe(source_state, src_gpu, partial_plan, required):
            _append_partial_reconfiguration_actions(
                actions,
                plan_items,
                source_state,
                target_state,
...
def _append_partial_reconfiguration_actions(
    ...
    gpu_id: int,
    physical_id: str,
    partial_plan: PartialReconfigPlan,
) -> None:
    src_gpu = gpu_map_by_id(source_state)[gpu_id]
    ...
    actions.extend(
        [
            ...
            _action(
                ...
                logical_gpu_id=gpu_id,
                pendingLogicalGpuId=gpu_id,
                template=partial_plan.target_template,
                partial=True,
                **fields,
                **common,
            ),
...
def _partial_reconfiguration_capacity_safe(
    source_state: ClusterState,
    src_gpu: GPUState,
    partial_plan: PartialReconfigPlan,
    required: dict[str, float],
    ...
    delete_instances = [
        inst
        for inst in _nonfree_instances(src_gpu)
        if (inst.start, inst.end, inst.profile) in delete_slots
    ]
    return safe_after_removing_instances(source_state, delete_instances, required)
...
def _build_initial_available_pool(source_state: ClusterState, active_src_map: dict[int, Any]) -> list[str]:
    ...
    observed_free_pool = [
        str(physical_id)
        for physical_id in source_state.metadata.get("free_physical_gpu_pool", [])
        if physical_id is not None and str(physical_id) not in active_pids
    ]
    available_existing = [
    ...
    if str(source_state.metadata.get("source", "")).startswith("go-cluster-state-manager"):
        return available_existing + list(reversed(observed_free_pool))
    never_seen = [
        physical_id
        for physical_id in reversed(PHYSICAL_ID_POOL)
        if physical_id not in active_pids and physical_id not in available_existing and physical_id not in observed_free_pool
    ]
    if observed_free_pool:
        # alloc_from_free_pool() pops from the end, so reverse to consume the
        # observed cluster pool in the order provided by cluster-state-manager.
        return never_seen + available_existing + list(reversed(observed_free_pool))
