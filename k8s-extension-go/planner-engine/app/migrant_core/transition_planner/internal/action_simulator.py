...
def _drain_map(state: ClusterState) -> dict[str, int]:
    ...
    return state.metadata.setdefault("transition_draining_instances", {})


def _runtime_map(state: ClusterState) -> dict[str, dict[str, Any]]:
    ensure_state_metadata(state)
    return state.metadata.setdefault("transition_runtime_instances", {})


def _reconfig_map(state: ClusterState) -> dict[str, dict[str, Any]]:
    ensure_state_metadata(state)
    return state.metadata.setdefault("transition_reconfig_targets", {})


def _target_geometry_only_gpu(target_gpu: GPUState) -> GPUState:
    gpu = copy.deepcopy(target_gpu)
    for inst in gpu.instances:
        inst.workload = None
        inst.batch = None
        inst.model_key = None
...
def _target_activation_actions(
    gpu_id: int,
    physical_id: str,
    target_gpu: GPUState,
    slots: list[tuple[int, int, str]] | None = None,
) -> list[dict[str, Any]]:
    wanted_slots = {tuple(slot) for slot in slots or []}
    actions = [_action("register_mig_devices", gpu_id=gpu_id, physical_gpu_id=physical_id)]
    for inst in _nonfree_instances(target_gpu):
        ...
        actions.extend(
            [
                _action("place_instance", gpu_id=gpu_id, physical_gpu_id=physical_id, slot=slot, workload=inst.workload, batch=inst.batch),
                _action("activate_instance_route", gpu_id=gpu_id, physical_gpu_id=physical_id, slot=slot, workload=inst.workload),
            ]
        )
    return actions


def _action(action_type: str, **kwargs: Any) -> dict[str, Any]:
...
def _abstract_action_label(action_type: str) -> str:
    return {
        ...
        "activate_instance_route": "Activate Route",
        "deactivate_instance_route": "Stop Traffic",
        "wait_instance_drain": "Wait Drain",
...
def prepare_transition_runtime(
    ...
    for gpu_id in sorted(set(src_map) | set(tgt_map)):
        ...
        if change in {"remove_gpu", "reconfiguration"} and src_gpu is not None:
            ...
            for inst_action in diff_instances_within_same_template(src_gpu, tgt_gpu):
                if inst_action["type"] in {"safe_delete_instance", "workload_change"} and inst_action.get("src") is not None:
...
def _active_pid_set(state: ClusterState) -> set[str]:
    active = {
        physical_id
        for _, physical_id in sorted(state.metadata.get("physical_id_map", {}).items(), key=lambda item: str(item[0]))
...
def _peak_from_actions(state_before: ClusterState, executed_actions: list[dict[str, Any]]) -> int:
    ...
    for action in executed_actions:
        ...
        if action_type == "allocate_gpu":
            ...
            if physical_id is not None:
                active.add(physical_id)
        elif action_type in {"clear_gpu", "clear_gpu_binding", "return_gpu"}:
            physical_id = action.get("physical_gpu_id")
...
def _reroute_destination_candidates(
    source_state: ClusterState,
    target_state: ClusterState,
    workload: str,
    ...
    def stable_serving_slot(gpu: GPUState, target_gpu: GPUState | None, slot: tuple[int, int, str]) -> bool:
        if target_gpu is None:
            return False
        if classify_gpu_change(gpu, target_gpu) not in {"keep_gpu", "instance_diff"}:
            return False
    ...
    for gpu in source_state.real_gpus():
        ...
        for inst in _nonfree_instances(gpu):
            ...
            if inst.workload != workload:
                continue
            if stable_serving_slot(gpu, target_gpu, slot):
                key = (gpu.gpu_id, slot)
                seen.add(key)
    ...
    for gpu in target_state.real_gpus():
        for inst in _nonfree_instances(gpu):
            ...
            if record is None:
                continue
            source_gpu = source_map.get(gpu.gpu_id)
            if source_gpu is not None and stable_serving_slot(source_gpu, gpu, slot):
                continue
            out.append(
                {
                    "kind": "target-backed",
...
def _reroute_destination_label(candidates: list[dict[str, Any]]) -> str | None:
    if not candidates:
        return None
...
def simulate_transition_actions(
    source_state: ClusterState,
    target_state: ClusterState,
    ...
    target_map = gpu_map_by_id(target_state)
    executed_state.metadata["transition_draining_instances"] = dict(_drain_map(source_state))
    executed_state.metadata["transition_runtime_instances"] = copy.deepcopy(_runtime_map(source_state))
    executed_state.metadata["transition_reconfig_targets"] = copy.deepcopy(_reconfig_map(source_state))
    ...
    for action in fine_actions:
        ...
        if action_type == "configure_full_template":
            ...
            physical_id = action.get("physical_gpu_id")
            if gpu_id is not None and physical_id is not None and get_physical_id(executed_state, int(gpu_id)) != physical_id:
        ...
        if action_type == "bind_target_gpu":
            ...
            if target_gpu is None:
                continue
            replace_or_append_gpu(executed_state, _target_geometry_only_gpu(target_gpu))
        ...
        if action_type in {"observe_mig_devices", "register_mig_devices"}:
            ...
            observed[str(gpu_id)] = {
                "physical_gpu_id": physical_id,
                "source": "simulated-device-plugin",
            }
            continue
        ...
        if action_type == "activate_instance_route":
            ...
            if cur_gpu is not None:
                for inst in _instances_for_action_slots(cur_gpu, action):
                    ...
                    runtime.setdefault("inflight", 0)
                    runtime["accepting_new"] = True
            active = executed_state.metadata.setdefault("active_serving_routes", {})
            active[str(gpu_id)] = {
                "physical_gpu_id": physical_id,
                "status": "active",
            }
        ...
        if action_type == "delete_instance":
            ...
            cur_gpu = get_gpu_by_id_mut(executed_state, gpu_id)
            if cur_gpu is not None:
                wanted_slots = _slots_from_delete_instance_action(action)
                for inst in list(cur_gpu.instances):
                    slot = (inst.start, inst.end, inst.profile)
                    if wanted_slots and slot not in wanted_slots:
                        continue
                    copy_inst_payload(inst, None)
                    _clear_runtime_entry(executed_state, gpu_id, slot)
                    _clear_drain(executed_state, gpu_id, slot)
            continue

        if action_type in {"clear_gpu", "clear_gpu_binding"}:
            gpu_id = int(action["gpu_id"])
            cur_gpu = get_gpu_by_id_mut(executed_state, gpu_id)
            if cur_gpu is not None:
                for inst in list(cur_gpu.instances):
        ...
        if action_type in {"update_batch", "apply_batch", "place_instance"}:
...
def _slots_from_delete_instance_action(action: dict[str, Any]) -> set[tuple[int, int, str]]:
...
def _remove_overlapping_void_slots(gpu: GPUState, start: int, end: int) -> None:
    for inst in list(gpu.instances):
        if str(inst.profile) != "void":
            continue
        if int(inst.end) <= start or int(inst.start) >= end:
            continue
        gpu.instances.remove(inst)


def _instances_for_action_slots(gpu: GPUState, action: dict[str, Any]) -> list[MigInstance]:
    slots = _slots_from_delete_instance_action(action)
    if not slots:
...
def plan_full_action_plan(
    ...
    source_state = prepare_transition_runtime(source_state, target_state)
    ...
    for gpu_id in all_gpu_ids:
        ...
        if change == "create_gpu":
            ...
            capacity_actions.extend(
                [
                    ...
                    *_target_activation_actions(gpu_id, physical_id, tgt_gpu),
                ]
            )
            plan_items.append(
                _plan_item(
                    ...
                    [
                        *cleanup_delete_actions,
                        ...
                        _action(
                            ...
                            gpu_id=gpu_id,
                            physical_gpu_id=physical_id,
                            deleteSlots=[(inst.start, inst.end, inst.profile) for inst in _nonfree_instances(src_gpu)],
                        ),
                        _action(
                            "return_gpu",
                            gpu_id=gpu_id,
        ...
        if change == "reconfiguration":
            ...
            in_place_ok = safe_after_removing_gpu(source_state, src_gpu, required)
            if record is None and in_place_ok:
                ...
                if in_place_ok:
                    finalize_actions.extend(
                        [
                            ...
                            *_target_activation_actions(gpu_id, old_physical_id, tgt_gpu),
                        ]
                    )
                    plan_items.append(
                        _plan_item(
                    ...
                    finalize_actions.extend(
                        [
                            ...
                            *_target_activation_actions(gpu_id, new_physical_id, tgt_gpu),
                            _action("clear_template", gpu_id=gpu_id, physical_gpu_id=old_physical_id),
                            _action(
                                "return_gpu",
                                gpu_id=gpu_id,
                                physical_gpu_id=old_physical_id,
        ...
        if change == "instance_diff":
            ...
            for inst_action in inst_actions:
                ...
                if inst_action["type"] == "place_instance":
                    ...
                    continue
                if inst_action["type"] == "safe_delete_instance":
...
def _reroute_destination_ready_score(source_state: ClusterState, target_state: ClusterState, item: dict[str, Any]) -> int:
    ...
    candidates = _reroute_destination_candidates(source_state, target_state, workload, exclude_gpu_id=gpu_id, exclude_slot=slot)
    score = 0
...
def _unlock_count_score(items: list[dict[str, Any]], item: dict[str, Any]) -> int:
    ...
    phase = str(item.get("current_phase", ""))
    if item_type == "create_gpu":
        return sum(1 for other in items if other.get("blocked_by") == "no_reroute_destination_capacity")
    ...
    if item_type in {"remove_gpu", "remove_gpu_slot", "workload_change", "delete_instance"} and "drain" in phase:
        return 1
    return 0
...
def _release_gpu_score(item: dict[str, Any]) -> int:
    ...
    if item_type == "reconfiguration" and phase in {"clear_old_side", "in_place_reconfigure", "finalize_target_side"}:
        return 2
    if item_type in {"workload_change", "delete_instance", "remove_gpu_slot", "reconfiguration_slot"} and phase == "post_drain_ready":
        return 1
    return 0
...
def score_plan_item(
    ...
    reroute_destination_ready = _reroute_destination_ready_score(source_state, target_state, item)
    ...
    return {
        ...
        "score": total,
        "reroute_destination_ready": reroute_destination_ready,
...
def _group_scores(
    ...
    scored = [score_plan_item(source_state, target_state, plan_items, item) for item in plan_items]
    groups = {}
...
def _action_matches_root(action: dict[str, Any], root_id: str) -> bool:
    ...
    if root_id.startswith(("WC_gpu", "RM_gpu", "PLACE_gpu", "BATCH_gpu")):
        ...
        if action.get("from_gpu_id") == gpu_id and tuple(action.get("from_slot", (None, None, None))) == slot:
            return True
