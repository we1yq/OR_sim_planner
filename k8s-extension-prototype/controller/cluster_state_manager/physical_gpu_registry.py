...
from cluster_state_manager.observed_layout import logical_mig_slots_for_binding
...
def build_physical_gpu_registry(
    ...
    observed_bindings = _merge_cached_bindings_when_inventory_empty(
        observed_state=observed_state,
        observed_bindings=observed_bindings,
        previous_status=previous_status,
    )
    ...
    for physical_id in discovered:
        ...
        available_eligible, availability_reason = _available_eligibility(
            ...
            mig_config=mig_config,
            mig_config_state=mig_config_state,
            mig_devices_fresh=bool(binding.get("migDevicesFresh", True)),
            policy=policy,
        )
...
def _merge_cached_bindings_when_inventory_empty(
    observed_state: dict[str, Any],
    observed_bindings: dict[str, dict[str, Any]],
    previous_status: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    if observed_bindings:
        return observed_bindings
    if list(observed_state.get("gpuOperatorInventory", [])):
        return observed_bindings
    previous_bindings = {
        str(physical_id): dict(binding)
        for physical_id, binding in dict(previous_status.get("bindings", {})).items()
    ...
    observed_nodes = {
        ...
        for row in list(observed_state.get("nodeInventory", []))
        if row.get("nodeName")
    }
    merged = dict(observed_bindings)
    for physical_id, cached in previous_bindings.items():
        node_name = str(cached.get("nodeName") or "")
        if observed_nodes and node_name not in observed_nodes:
            continue
        if not _is_a100_product(cached.get("product")):
            continue
        merged[physical_id] = {
            **cached,
            "physicalGpuId": physical_id,
            "migDevicesFresh": False,
            "bindingSource": "previous-registry-cache-when-gpu-operator-inventory-empty",
            "confidence": "cached-gpu-uuid",
        }
    return merged
...
def mark_physical_gpu_mig_cdi_ready(
    ...
    _normalize_queues_from_bindings(status)
...
def _available_eligibility(
    ...
    if mig_devices_fresh and mig_config_state == "success" and str(mig_config or "").startswith("or-sim-"):
        return True, "per_device_empty_config_success"
...
def _normalize_queues_from_bindings(status: dict[str, Any]) -> None:
    bindings = {
        str(physical_id): dict(binding)
        for physical_id, binding in dict(status.get("bindings", {})).items()
    }
    discovered = sorted(
        ...
        if binding.get("product") is None or _is_a100_product(binding.get("product"))
    )
    active = []
    available = []
    transitioning = []
    for physical_id in discovered:
        state = str(bindings[physical_id].get("state") or "")
        if state == "active":
            active.append(physical_id)
        elif state == "transitioning":
            transitioning.append(physical_id)
        elif state == "available":
            available.append(physical_id)
        elif bindings[physical_id].get("availableEligible") is True:
            available.append(physical_id)
        else:
            transitioning.append(physical_id)
    status["discoveredA100"] = discovered
    status["activeQueue"] = active
    status["availableQueue"] = available
    status["transitioningQueue"] = transitioning
    status["queueCounts"] = _queue_counts(status)
