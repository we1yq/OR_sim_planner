...
class PythonKubernetesClient:
    ...
    def get_arrivalsnapshot(self, name: str, namespace: str) -> dict[str, Any]:
        obj = self.custom.get_namespaced_custom_object(
            ...
            version=self.version,
            namespace=namespace,
            plural="arrivalsnapshots",
            name=name,
        )
        if not isinstance(obj, dict):
            raise ValueError(f"Kubernetes API returned a non-object ArrivalSnapshot for {namespace}/{name}")
        return obj
    ...
    def list_configmaps(self, namespace: str) -> list[dict[str, Any]]:
        obj = self.core.list_namespaced_config_map(namespace=namespace)
        serialized = self.api_client.sanitize_for_serialization(obj)
        if not isinstance(serialized, dict):
            raise ValueError(f"Kubernetes API returned a non-object ConfigMapList for namespace {namespace}")
        return list(serialized.get("items", []))
