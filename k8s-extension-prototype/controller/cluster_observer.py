...
from observed_layout import logical_mig_slots_from_bindings
    ...
    for row in gpu_inventory:
        for binding in _bindings_from_gpu_operator_inventory(row):
            if not _is_a100_product(binding.get("product")):
                ignored.append({**binding, "reason": "non-A100 GPU ignored by OR-SIM MIG planner"})
                continue
            ...
            ignored.append(
                {
                    ...
                    "confidence": "node-device-index-inferred",
                    "reason": "non-A100 or unknown aggregate GPU capacity ignored by OR-SIM MIG planner",
                }
            )
    return {
        "bindings": bindings,
