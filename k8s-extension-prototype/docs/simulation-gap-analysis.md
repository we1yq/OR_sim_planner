...
The V3 planner depends on V2-generated candidate plan items. Those plan items
...
The public planner should remain V3, but V3 still needs candidate items:
...
The notebook currently gets these from `plan_naive_action_v2`. The prototype
...
It is an internal V3 dependency, not a V2 public planner.

### 9. V3 Scoring Is Incomplete

The notebook V3 score uses:
...
stage1 GPU count: 9
stage2 GPU count: 6
stage3 GPU count: 8 in the sequential V3 path
```
...
- better-than-V2 remove/create demo.
