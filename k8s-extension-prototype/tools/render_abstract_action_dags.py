...
DIAGRAMS: list[dict[str, Any]] = [
    {
        ...
        "nodes": [
            ...
            ("observe-mig", "Observe MIG devices: map target slots to real MIG device UUIDs", "mig", 3, 0),
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("observe-new", "Observe MIG devices: map target slots to real MIG device UUIDs", "mig", 5, 0),
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("drain", "Wait Drain: wait until queued requests and running work are zero", "router", 2, 1, "optional"),
            ...
            ("observe-mig", "Observe MIG devices: map target slots to real MIG device UUIDs", "mig", 7, 1),
    ...
    },
    {
        ...
        "description": "Patch only changed MIG slots on an active GPU; preserved slots keep serving while new slots are observed and deployed.",
        "nodes": [
            ("accept", "Accept Queued Requests: stable serving slot accepts rerouted queued requests", "router", 1, 0, "optional"),
            ("stop", "Stop Slot Traffic: stop new requests entering pods on deleted slots", "router", 0, 1),
            ("reroute", "Reroute Queued Requests: move queued requests from deleted slots to stable serving slots", "router", 1, 1, "optional"),
            ("drain", "Wait Drain: wait until queued requests and running work are zero on deleted slots", "router", 2, 1, "optional"),
            ("delete-pods", "Delete Pods: delete workload pods only on deleted slots", "cleanup", 3, 1),
            ("patch", "Partial Reconfiguration: patch MIG geometry using delete/create/preserve slot specs", "mig", 4, 1),
            ("observe", "Observe MIG devices: map new slots to MIG UUIDs and confirm preserved slots still exist", "mig", 5, 1),
            ("deploy", "Deploy Pods: deploy workload pods only on newly created target slots", "pod", 6, 1),
            ("route", "Activate Route: route new requests to newly created target pods", "router", 7, 1),
        ],
        "edges": [
            ("accept", "reroute"),
            ("stop", "reroute"),
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("drain-bridge", "Drain Bridge: wait until bridge queued requests and running work are zero", "router", 3, 0, "optional"),
            ...
            ("drain", "Wait Drain: wait until original pod running work is zero", "router", 2, 1, "optional"),
            ("remove", "Delete Pod: delete old workload pod", "cleanup", 3, 1),
            ("place", "Deploy Pod: deploy replacement workload pod on same slot", "pod", 4, 1),
            ("route", "Activate Route: route new requests to replacement pod", "router", 5, 1),
        ],
