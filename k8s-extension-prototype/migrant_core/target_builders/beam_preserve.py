...
from ..preserve import reassign_gpu_ids_by_matching
from ..target_builder import _apply_same_logical_template_order_fix
...
from ..target_materialization import _score_tuple, _solve_target_with_preserve_first_beam


NAME = "target.beam_preserve"
...
def build(
    ...
    if best_target is None:
        raise RuntimeError("Beam target-state build failed: no feasible candidate found")
    ...
    best_target.metadata["build_metrics"]["beam_width"] = int(beam_width)
    best_target.metadata["build_metrics"]["slot_choice_width"] = int(slot_choice_width)
    best_target.metadata["build_method"] = "beam"
    best_target.metadata["target_builder_module"] = NAME

    if verbose:
        print("=" * 90)
        print("[TARGET-STATE BUILDER] BEST RESULT (beam)")
        print("=" * 90)
